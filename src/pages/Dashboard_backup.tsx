...
// 从本地存储获取数据并处理
 const getDashboardData = (timeRange: 'today' | 'week' | 'month' | 'year') => {
  ...
  // 计算本月消费总额
  const monthlyConsumptions = getConsumptionRecords().filter(c => {
  ...
  const revenueData = [];
   ...
   if (timeRange === 'today') {
    ...
    // 计算今日的消费总额
    const todayConsumptions = getConsumptionRecords().filter(c => {
    ...
    const revenueData = hours.map(hour => {
      ...
      const hourRecharges = todayRecharges.filter(r => {
        ...
        return Math.floor(hour / 3) === hours.indexOf(hour);
      });
      ...
      const hourMembers = members.filter(member => {
        ...
        return Math.floor(joinHour / 3) === hours.indexOf(hour);
      });
    ...
    return {
      ...
      monthlyConsumptionAmount: getConsumptionRecords().reduce((sum, c) => sum + c.amount, 0),
      totalMembers: members.length,
      cardTypesCount: cardTypes.length,
      timeRange
    };
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      // 计算该月的消费总额
      const consumptionTotal = getConsumptionRecords()
        .filter(c => {
...
// 消费时段分布数据
 ...
 // 最近消费记录 - 从实际数据中获取
 const getRecentConsumptionData = () => {
 ...
 // 格式化最近时间显示
 const formatRecentTime = (timeString: string) => {
 ...
 const recentConsumptionData = getRecentConsumptionData();

 // 最近充值记录 - 从实际数据中获取
 const getRecentRechargeData = () => {
 ...
 const recentRechargeData = getRecentRechargeData();

 // 即将过期的次卡 - 从实际数据中获取
 const getExpiringCardsData = () => {
  const members = getMembers();
  const expiringCards = [];
 ...
 const expiringCardsData = getExpiringCardsData();
...
const StatCard = ({ 
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setIsIncreased(change.startsWith('+'));
    }, 500);
...
export default function Dashboard() {
  ...
  <div className="flex items-center space-x-2">
    <button className={`px-4 py-2 rounded-lg text-sm font-medium transition duration-200 ${timeRange === 'today' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'}`} onClick={() => setTimeRange('today')}>今日</button>
    <button className={`px-4 py-2 rounded-lg text-sm font-medium transition duration-200 ${timeRange === 'week' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'}`} onClick={() => setTimeRange('week')}>本周</button>
    <button className={`px-4 py-2 rounded-lg text-sm font-medium transition duration-200 ${timeRange === 'month' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'}`} onClick={() => setTimeRange('month')}>本月</button>
    <button className={`px-4 py-2 rounded-lg text-sm font-medium transition duration-200 ${timeRange === 'year' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'}`} onClick={() => setTimeRange('year')}>全年</button>
        </div>
        <div className="flex items-center space-x-2">
          <button onClick={handleExportData} className="px-4 py-2 bg-white text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-100 transition duration-200 flex items-center">
            <i className="fa-solid fa-download mr-2"></i>导出数据
          </button>
          <button onClick={handleRefresh} className="px-4 py-2 bg-white text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-100 transition duration-200 flex items-center">
              ...
              <div className="card rounded-xl shadow-sm border p-6 bg-white hover:shadow-md transition-shadow duration-300">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-lg font-semibold text-gray-900">
                    {timeRange === 'month' ? '近6个月' : '全年'}营业额与充值趋势
                  </h2>
             ...
             <div className="lg:col-span-2 card rounded-xl shadow-sm border p-6">
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead>
                    <tr>
                      <th className="px-3 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider rounded-tl-lg">会员</th>
                      <th className="px-3 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">时间</th>
                      <th className="px-3 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">服务项目</th>
                      <th className="px-3 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">金额</th>
                      <th className="px-3 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider rounded-tr-lg">状态</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {recentConsumptionData.map((item) => (
                      <tr key={item.id} className="hover:bg-gray-50 transition duration-150">
                        <td className="px-3 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{item.member}</td>
                        <td className="px-3 py-4 whitespace-nowrap text-sm text-gray-500">{item.time}</td>
                        <td className="px-3 py-4 whitespace-nowrap text-sm text-gray-700">{item.service}</td>
                        <td className="px-3 py-4 whitespace-nowrap text-sm font-medium text-gray-900">¥{item.amount}</td>
